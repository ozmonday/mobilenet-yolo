...
class MNetYolo():
    ...
    def predict(self, img_path, plot_img=True):
        img_ori = utillity.open_image(img_path)
    ...
    def export_gt(self, image_notation, image_folder_path, export_path):
        for line in image_notation:
            ...
            img = utillity.open_image(filename)
    ...
    def export_predict(self, image_anotation, image_folder_path, export_path, betch_size=2):
        ...
        for idx in tqdm(range(0, len(img_paths), betch_size)):
            ...
            for i, path in enumerate(paths):
                img = utillity.open_image(path, False)
    ...
    def eval_map(self, gt_folder_path, pred_folder_path, temp_json_folder_path, output_files_path):
        ...
        for txt_file in ground_truth_files_list:
            ...
            lines_list = utillity.read_txt_to_list(txt_file)
            ...
            for txt_file in dr_files_list:
                ...
                if class_index == 0:
                    if not os.path.exists(temp_path):
                        ...
                        print(error_msg)
                lines = utillity.read_txt_to_list(txt_file)
                for line in lines:
                    try:
        ...
        with open(output_files_path + "/output.txt", 'w') as output_file:
            ...
            for class_index, class_name in enumerate(gt_classes):
                ...
                ap, mrec, mprec = utillity.voc_ap(rec[:], prec[:])
                sum_AP += ap
                text = "{0:.2f}%".format(
